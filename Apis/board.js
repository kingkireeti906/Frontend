...
const backendUrl = `https://protaskmanagerbackend.onrender.com`;
...
export const updateEditData = async (
  { _id },
  { title, priority, checklist, dueDate, vp }
) => {
  try {
    const reqUrl = `${backendUrl}/api/v1/board/edit/${_id}`;
    const reqPayload = { title, priority, checklist, dueDate, vp };
...
export const updateduetask = async ({ date }) => {
  try {
    const reqUrl = `${backendUrl}/api/v1/board/updateduetask`;
    console.log(reqUrl);
